...
export class Orchestrator {
  ...
  async deploy(params: any): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      exec('aws eks --region us-east-1 update-kubeconfig --name $cluster_name && terraform init && terraform plan && TF_LOG=DEBUG terraform apply -auto-approve', { cwd: this.terraformPath }, (error, stdout, stderr) => {
        if (error) {
